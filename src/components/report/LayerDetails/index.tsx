...
export function LayerDetails({ layer1, layer2, layer3, layer4, aiAnalysis }: LayerDetailsProps) {
  ...
  return (
    <section aria-labelledby="layers-heading">
      ...
      {/* Layer 1 */}
      <AccordionPanel title="Fact-check-uri anterioare" count={layer1.results.length} status={layer1.status}>
        {layer1.results.length === 0 ? (
          ...
          <ul className={styles.resultList}>
            {layer1.results.map((r, i) => (
              <li key={i} className={styles.resultItem}>
                <span className={`${styles.rating} ${r.ratingValue < 0.4 ? styles.ratingFalse : r.ratingValue > 0.6 ? styles.ratingTrue : styles.ratingPartial}`}>
                  {r.rating}
                </span>
                <span className={styles.resultText}>{r.claimReviewed.slice(0, 120)}</span>
                <a href={r.reviewUrl} target="_blank" rel="noopener noreferrer" className={styles.link}>{r.publisher} →</a>
              </li>
            ))}
          </ul>
        )}
      ...
      {/* Layer 2 */}
      <AccordionPanel title="Articole jurnalistice" count={layer2.results.length} status={layer2.status}>
        {layer2.results.length === 0 ? (
          <p className={styles.emptyMsg}>Nu au fost gasite articole relevante.</p>
        ) : (
          <ul className={styles.resultList}>
            {layer2.results.map((a, i) => (
              <li key={i} className={styles.resultItem}>
                <span className={`${styles.sentiment} ${styles[`sentiment_${a.sentiment}`]}`}>
                  {a.sentiment === 'confirms' ? '✓ Confirma' : a.sentiment === 'contradicts' ? '✗ Contrazice' : '• Neutru'}
                </span>
                <span className={styles.resultText}>{a.title}</span>
                <a href={a.articleUrl} target="_blank" rel="noopener noreferrer" className={styles.link}>{a.source} →</a>
              </li>
            ))}
          </ul>
        )}
      ...
      {/* Layer 3 */}
      <AccordionPanel title="Surse oficiale" count={layer3.results.length} status={layer3.status}>
        {layer3.results.length === 0 ? (
          <p className={styles.emptyMsg}>Nu au fost gasite documente oficiale relevante.</p>
        ) : (
          <ul className={styles.resultList}>
            {layer3.results.map((o, i) => (
              <li key={i} className={styles.resultItem}>
                <strong className={styles.org}>{o.organization}</strong>
                <span className={styles.resultText}>{o.relevantQuote.slice(0, 150)}</span>
                <a href={o.documentUrl} target="_blank" rel="noopener noreferrer" className={styles.link}>Document oficial →</a>
              </li>
            ))}
          </ul>
        )}
      ...
      {/* Layer 4 */}
      <AccordionPanel title="Declaratii publice" count={layer4.results.length} status={layer4.status}>
          ...
          <ul className={styles.resultList}>
            {layer4.results.map((p, i) => (
              <li key={i} className={styles.resultItem}>
                ...
                <span className={styles.resultText}>{p.content.slice(0, 200)}</span>
                <a href={p.postUrl} target="_blank" rel="noopener noreferrer" className={styles.link}>{p.platform} →</a>
              </li>
            ))}
          </ul>
        )}
